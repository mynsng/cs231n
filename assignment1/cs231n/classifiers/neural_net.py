...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        nor_scores = exp_scores / np.sum(exp_scores, axis = 1, keepdims= True)

        correct_class_score = nor_scores[np.arange(N), y]
        ...
        grads['W2'] = np.dot(B.T, dscores)
        grads['b2'] = np.sum(dscores, axis=0)

        dB = np.dot(dscores, W2.T)
    ...
    def predict(self, X):
        ...
        scores = np.dot(np.maximum((np.dot(X,W1)+b1),0),W2)+b2
